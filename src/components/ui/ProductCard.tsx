  ...
  return (
    <article className={`group relative bg-card border border-white/5 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_48px_rgba(0,0,0,0.65)] hover:border-green-mid/30`}>
      ...
      {product.badge && (
        ...
        `}>
          {product.badge === "bestseller" ? "Best Seller" : product.badge === "discounted" ? "Discounted" : product.badge}
        </span>
      )}
      ...
      <div className="p-4">
        ...
        {/* Star rating — social proof (hidden when the product has no reviews) */}
        {product.rating > 0 && (
            ...
            <span className="flex items-center gap-[2px]">
              {[1, 2, 3, 4, 5].map((n) => (
                <svg
                  ...
                  height="11"
                  viewBox="0 0 24 24"
                  fill={n <= Math.round(product.rating) ? "#c9a84c" : "none"}
                  stroke="#c9a84c"
                  strokeWidth="1.5"
